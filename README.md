...
- **Admin (`/admin`)**: password-protected. Shows every event as a card with a booked/total count. From here:
  - **+ New Event** creates a new day: title, date, start/end time, slot length, location, and break start times. Break times are auto-suggested at :50 past each hour in your window, and you can edit or clear them before saving.
  - **Manage** opens the full booking list for one event, with a one-click Release for any slot, and a **+ Add More Time** panel to extend the day (adds new slots after the current end time, with its own auto-suggested breaks, without touching any existing bookings).
